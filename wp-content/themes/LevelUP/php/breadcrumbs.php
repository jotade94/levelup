	...
	function boldthemes_breadcrumbs( $simple = false ) {
		...
		if ( ! is_404() && ! is_home() ) {
			...
			if ( is_category() ) {
				...
				$title = esc_html__( 'Category:', 'eventim' ) . ' ' . single_cat_title( '', false );
				...
				$output .= $item_prefix . esc_html__( 'Portfolio', 'eventim' ) . $item_sufix;
				$output .= $item_prefix . get_the_title() . $item_sufix;
				...
				$title = esc_html__( 'Tag:', 'eventim' ) . ' ' . single_tag_title( '', false );
				$output .= $item_prefix . $title . $item_sufix;
		  
			} else if ( is_author() ) {
			
				$title = esc_html__( 'Author:', 'eventim' ) . ' ' . get_the_author_meta( 'display_name' );
				$output .= $item_prefix .  $title . $item_sufix;
				...
				$title = esc_html__( 'Search:', 'eventim' ) . ' ' . get_query_var( 's' );
				$output .= $item_prefix . $title . $item_sufix;			
