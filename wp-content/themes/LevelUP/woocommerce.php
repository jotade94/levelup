...
if ( is_product() ) {
	EventimTheme::$boldthemes_page_for_header_id = boldthemes_get_id_by_slug( boldthemes_get_option( 'shop_settings_page_slug' ) );
} else if ( ( is_shop() || is_product_category() || is_product_taxonomy() ) && get_option( 'woocommerce_shop_page_id' ) ) {
	EventimTheme::$boldthemes_page_for_header_id = get_option( 'woocommerce_shop_page_id' );
}
