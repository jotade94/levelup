...
if ( ! class_exists( 'EventimTheme' ) ) {
	...
	class EventimTheme {
		...
		function __construct() {
			...
			add_filter( 'wp_title', array( $this, 'boldthemes_title' ) );
		...
		function boldthemes_init() {
			...
			register_nav_menus( array (
				'primary' => esc_html__( 'Primary Menu', 'eventim' ),
				'footer'  => esc_html__( 'Footer Menu', 'eventim' )
			));
			
			// load translated strings
			load_theme_textdomain( 'eventim', get_template_directory() . '/languages' );
			
			// date format
			if ( function_exists( 'icl_register_string' ) ) {
				icl_register_string( 'eventim', 'Date Format', get_option( 'date_format' ) );
				EventimTheme::$boldthemes_date_format = icl_t( 'eventim', 'Date Format', get_option( 'date_format' ) );
			} else {
				EventimTheme::$boldthemes_date_format = get_option( 'date_format' );
			}
		...
		function boldthemes_set_global_uri() {
			...
			echo 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'eventim' ) . '\';';
			echo 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'eventim' ) . '\';';		
			echo '</script>';
		}
		...
		function boldthemes_widgets_init() {  
			...
			register_sidebar( array (
				'name' 			=> esc_html__( 'Sidebar', 'eventim' ),
		...
		function boldthemes_load_fonts() {
			...
			if ( $body_font != 'no_change' ) {
				...
				$body_font_state = _x( 'on', 'Raleway font: on or off', 'eventim' );
			...
			if ( $heading_font != 'no_change' ) {
				...
				$heading_font_state = _x( 'on', 'Roboto font: on or off', 'eventim' );
			...
			if ( $menu_font != 'no_change' ) {
				...
				$menu_font_state = _x( 'on', 'Raleway font: on or off', 'eventim' );
			...
			if ( $heading_subtitle_font != 'no_change' ) {
				...
				$heading_subtitle_font_state = _x( 'on', 'Roboto font: on or off', 'eventim' );
			...
			if ( $heading_supertitle_font != 'no_change' ) {
				...
				$heading_supertitle_font_state = _x( 'on', 'Roboto font: on or off', 'eventim' );
		...
		function boldthemes_theme_register_required_plugins() {
			...
			$plugins = array(
				...
				array(
					'name'               => esc_html__( 'Eventim', 'eventim' ), // The plugin name.
					'slug'               => 'eventim', // The plugin slug (typically the folder name).
					'source'             => get_template_directory() . '/plugins/eventim.zip', // The plugin source.
					'required'           => true, // If false, the plugin is only 'recommended' instead of required.
					'version'            => '1.1.7', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
					'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
					'force_deactivation' => true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
					'external_url'       => '', // If set, overrides default API URL and points to an external URL.
				),
				array(
					'name'               => esc_html__( 'Cost Calculator', 'eventim' ), // The plugin name.
				...
				array(
					'name'               => esc_html__( 'Rapid Composer', 'eventim' ), // The plugin name.
				...
				array(
					'name'               => esc_html__( 'BoldThemes WordPress Importer', 'eventim' ), // The plugin name.
				...
				array(
					'name'               => esc_html__( 'Meta Box', 'eventim' ), // The plugin name.
				...
				array(
					'name'               => esc_html__( 'Contact Form 7', 'eventim' ), // The plugin name.
				...
				array(
					'name'               => esc_html__( 'WooSidebars', 'eventim' ), // The plugin name.
			...
			$config = array(
				...
				'strings'      => array(
					'page_title'                      => esc_html__( 'Install Required Plugins', 'eventim' ),
					'menu_title'                      => esc_html__( 'Install Plugins', 'eventim' ),
					'installing'                      => esc_html__( 'Installing Plugin: %s', 'eventim' ), // %s = plugin name.
					'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'eventim' ),
					'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'eventim' ), // %1$s = plugin name(s).
					'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'eventim' ), // %1$s = plugin name(s).
					'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'eventim' ), // %1$s = plugin name(s).
					'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'eventim' ), // %1$s = plugin name(s).
					'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'eventim' ), // %1$s = plugin name(s).
					'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'eventim' ), // %1$s = plugin name(s).
					'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'eventim' ), // %1$s = plugin name(s).
					'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'eventim' ), // %1$s = plugin name(s).
					'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'eventim' ),
					'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'eventim' ),
					'return'                          => esc_html__( 'Return to Required Plugins Installer', 'eventim' ),
					'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'eventim' ),
					'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'eventim' ), // %s = dashboard link.
		...
		function boldthemes_search_form( $form ) {
			...
			<div class="btSearchInner" role="search">
				<div class="btSearchInnerContent">
					<form action="' . home_url( '/' ) . '" method="get"><input type="text" name="s" placeholder="' . esc_attr( esc_html__( 'Looking for...', 'eventim' ) ) . '" class="untouched">
	...
	$boldthemes_theme = new EventimTheme();
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'eventim' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'eventim' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'eventim' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'eventim' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'eventim' ), get_comment_date(), get_comment_time() ) . '</p>';
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'eventim' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'eventim' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'eventim' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! function_exists( 'boldthemes_get_posts_array_item' ) ) {
	function boldthemes_get_posts_array_item( $post_type = 'blog', $post_id, $post_author ) {
		...
		$post_data['date'] = date_i18n( EventimTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d', $post_id ) ) );
...
if ( ! function_exists( 'boldthemes_get_body_class' ) ) {
	function boldthemes_get_body_class() {
		...
		EventimTheme::$boldthemes_sidebar = boldthemes_get_option( 'sidebar' );

		if ( ! ( ( EventimTheme::$boldthemes_sidebar == 'left' || EventimTheme::$boldthemes_sidebar == 'right' ) && ! is_404() ) ) {
			EventimTheme::$boldthemes_has_sidebar = false;
			$extra_class[] = 'btNoSidebar';
		} else {
			EventimTheme::$boldthemes_has_sidebar = true;
			if ( EventimTheme::$boldthemes_sidebar == 'left' ) {
