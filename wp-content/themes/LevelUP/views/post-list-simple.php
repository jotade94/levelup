<?php
	...
	if ( $blog_author || $blog_date || $show_comments_number ) {
		$meta_html .= '';
		if ( $blog_date  && !$blog_side_info ) $meta_html .= '<span class="btArticleDate">' . esc_html( date_i18n( EventimTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>';
		if ( $blog_date  && $blog_side_info ) $categories_html .= '<span class="btArticleDate">' . esc_html( date_i18n( EventimTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>';
