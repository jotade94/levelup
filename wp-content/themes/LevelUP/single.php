...
EventimTheme::$boldthemes_page_for_header_id = boldthemes_get_id_by_slug( boldthemes_get_option( 'blog_settings_page_slug' ) );
...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		if ( '' != $prev ) {
			...
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'previous', 'eventim' ) . '</span><span class="nbsTitle">' . esc_html( $prev->post_title ) . '</span></span>';
			$prev_next_html .= '</a></h4>';
		}
		$prev_next_html .= '</div>';
		...
		if ( '' != $next ) {
			...
			$url = $thumb[0];
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'next', 'eventim' ) . '</span><span class="nbsTitle">' . esc_html( $next->post_title ) . '</span></span>';
			$style_el = '';
			if ($url != '') $style_el = ' style="background-image:url(\'' . $url . '\');"';
			$prev_next_html .= '<span class="nbsImage"><span class="nbsImgHolder"' . $style_el . '></span></span>';
			$prev_next_html .= '</a></h4>';
		...
		if ( has_post_thumbnail() && boldthemes_get_option( 'blog_ghost_slider' ) ) {
				...
				$author_html = '<a href="' . esc_url_raw( $author_url ) . '" class="btArticleAuthor">' . esc_html__( 'by', 'eventim' ) . ' ' . esc_html( get_the_author() ) . '</a>';				
				
				if ( $blog_author || $blog_date || $show_comments_number ) {
				
					$meta_slider_html .= '<p class="btSubTitle btArticleMeta">';
					
					if ( $blog_date ) $meta_slider_html .= '<span class="btArticleDate">' . esc_html( date_i18n( EventimTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . '</span>'; 
		...
		<?php }
			...
			if ( $blog_author || $blog_date || $show_comments_number ) {
				$meta_html .= '';
				if ( $blog_date ) $meta_html .= '<span class="btArticleDate">' . esc_html( date_i18n( EventimTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>'; 
